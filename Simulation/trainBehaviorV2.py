...
from tensorflow.python.keras.layers import GlobalAveragePooling2D, Flatten, Conv2D, AveragePooling2D
...
shouldRunTest = False
# create a tkinter button which sets shouldRunTest to True
root = tkinter.Tk()
...
def setTrue():
    global shouldRunTest
    shouldRunTest = True


button = tkinter.Button(root, text="Run Test", command=setTrue)
# show the button
button.pack()
...
def buildModel(input_shape, action_space):
    ...
    model = Conv2D(64, (2, 2), activation="relu")(X_input)
    model = Conv2D(64, (2, 2), activation="relu")(model)
    model = Conv2D(64, (2, 2), activation="relu")(model)
...
class PPOAgent:
    ...
    def train(self, states, actions, next_states):
        ...
        self.actor.fit(states, actions, epochs=self.epochs, batch_size=64, verbose=1, shuffle=self.shuffle)
        self.save()
...
def main(enableTrain=True):
    ...
    try:
        ...
        try:
            agent.load("CarEnv_PPO_Actor.h5")
            print("Model loaded...")
        except:
            pass
        if enableTrain:
...
if __name__ == "__main__":
    main()
    # newest gym fixed bugs in 'BipedalWalker-v2' and now it'imageSocket called 'BipedalWalker-v3'
