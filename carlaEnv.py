...
class CarEnv(Env):
    ...
    def getState(self):
        ...
        self.state = cv2.resize(self.state, dsize=(stateWidth, stateHeight)) / 255
    ...
    def step(self, action):
        ...
        if not self.autopilotEnabled:
            # act = [int((action[0] * 10)) / 10, int((action[1] - 0.5) * 20) / 10, 0]
            act = [max(0, float((action[0] * 10)) / 10), max(0, float((action[1]) * 10) / 10), float(action[2] * 10) / 10]
            #  print(int((action[0] * 10)) / 10, int((action[1] - 0.5) * 20) / 10)
            if act[2] < 0.1:
                act[2] = 0
        ...
        return self.state, reward, done, [control.throttle, control.steer, control.brake]
